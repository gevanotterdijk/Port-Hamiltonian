...
class coupled_MSD():
    def __init__(self, M_vals, D_vals, K_vals):
    ...
    def hamiltonian(self, x):
        ...
        # Momentum dependent grad Hamiltonian (i.e. Mp)
        p = x[self.n_sys:]
...
if __name__ == "__main__":
    ...
    sys = coupled_MSD(M_vals=M_vals, D_vals=D_vals, K_vals=K_vals)
    ...
    for i in range(0, n_datasets):
        ...
        if noise == True:
            ...
            noisy_output = (noise_sd*max_out*torch.randn_like(output) - 0.5*noise_sd*max_out*torch.ones_like(output)) + output
